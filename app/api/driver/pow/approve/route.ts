...
import { reviewDriverWorkEligibilityDocument } from "@/lib/admin-drivers-service"
...
export async function POST(request: NextRequest) {
  ...
  const result = await reviewDriverWorkEligibilityDocument(driverId, {
    documentId,
    action: reviewAction,
    reason,
    notes,
  ...
  if (!result.data) {
    ...
    return NextResponse.json(result.error ?? { message: "Work eligibility document review failed" }, { status: Number(result.error?.status) || 400 })
  }
  ...
  return NextResponse.json({
    ...result.data,
    message: result.data?.message || (reviewAction === "APPROVED" ? "Proof of work document approved successfully" : "Proof of work document rejected successfully"),
  })
}
