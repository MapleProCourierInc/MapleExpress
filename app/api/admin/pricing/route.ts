...
function validatePayload(raw: Record<string, unknown>) {
  ...
  ;(payload.surcharges || []).forEach((surcharge, index) => {
    ...
    if (!["FLAT", "PER_KM", "PERCENTAGE"].includes(surcharge.calculationType)) add(`surcharges.${index}.calculationType`, "Use FLAT, PER_KM, or PERCENTAGE for surcharges")
    ...
    if (surcharge.enabled && ["FLAT", "PER_KM"].includes(surcharge.calculationType) && !nonNegative(surcharge.amount)) add(`surcharges.${index}.amount`, "Enabled flat and per-kilometre surcharges require an amount")
