...
export function CustomerBillingTable({ rows }: { rows: AdminCustomerBillingRow[] }) {
  return (
    <div className="space-y-2">
      <p className="text-sm text-muted-foreground">Showing {rows.length} customer profiles</p>
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={`${row.ownerType}-${row.id}`}>
                ...
                <TableCell className="max-w-[240px] truncate text-sm text-muted-foreground">{row.secondaryInfo || "—"}</TableCell>
